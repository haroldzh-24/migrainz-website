...
import { getProject, projectHref, chapterHref } from "@/data/projects";
export async function generateMetadata({
  ...
  return {
    title:
      getProject(ids.slug)?.characters.find((c) => c.slug === ids.character)
...
export default async function CharacterPage({
  ...
  const ids = await params;
  const project = getProject(ids.slug);
  if (!project) notFound();
